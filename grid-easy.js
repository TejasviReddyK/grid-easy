...
function setUpTheGrid(){
  ...
  gridContainer.innerHTML=document.getElementById('tmpl-grid-easy').innerHTML;
  editorTabs.innerHTML=document.getElementById('tmpl-grid-easy-buttons').innerHTML;
  ...
  document.getElementById('grid-easy-edit-area-button').addEventListener('click',updateBlock);
...
function keyboardGridActions(e){
	...
	switch(e.which){
		case 9:
			...
			if(target.matches('.row-control-list button:last-child,.column-control-list button:last-child')){
		...
		case 27:
			...
			break;
		case 40:
			if(target.matches('.row-control-list button,.column-control-list button')){
				e.preventDefault();
...
function edit(el){
	...
	var controls=dupe.querySelectorAll('.row-control,.column-control');
        for(var i=0,l=controls.length;i<l;i++){controls[i].parentElement.removeChild(controls[i]);}
...
function duplicate(el){
	...
	el.nextElementSibling ? el.parentElement.insertBefore(dupe,el.nextElementSibling) : el.parentElement.appendChild(dupe);
	setTimeout(function(){dupe.classList.remove('highlight');},200);
}
...
function prepareGrid(){
	...
	colControl=document.getElementById('tmpl-grid-easy-column-control').innerHTML;
	rowControl=document.getElementById('tmpl-grid-easy-row-control').innerHTML;
	addControls(rows);
